from tkinter import *
#lets comment this at some point!
class App:

    def __init__(self, master):
        self._master = master
        self._btn_matrix = [] #keeps all matrix buttons together using the same 'button=' reference when initializing
    ...
    def updateChange(self,x,y):
        ...
        if xY not in self.listButtonsPressed:
            ...
            self._btn_matrix[7-x][7-y].configure(bg="#000000")

        elif xY in self.listButtonsPressed:
            self.listButtonsPressed.remove(xY)
            self._btn_matrix[7-x][7-y].configure(bg="#ffffff")
